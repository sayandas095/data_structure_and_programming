...
{
	long int l1,l2,i,med=(l+u)/2,b[size];
...
void mergesort(long int a[],long int l,long int u,long int size, total_comparision *tc)
{
	long int med;
	if(l>=u)
		return;
	else
	{
		med=(l+u)/2;
		tc->count += 1;
		mergesort(a,l,med,size, tc);
		mergesort(a,med+1,u,size, tc);
		merge(a,l,u,size);
	}
}
...
void main()
{
	...
	printf("\nTable for merge sort (values given in terms of comparision) :-\n");
	printf("---------------------------------------------------------\n");
